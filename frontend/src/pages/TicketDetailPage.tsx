...
export function TicketDetailPage() {
  ...
  const canAcceptWork = current.status === 'PENDING' && isAssignedTechnician
      ...
      <div className="page-header">
        <div>
          ...
          <div className="d-flex flex-wrap gap-2">
            ...
            <span className={`badge-status badge-${current.status.toLowerCase()}`}>{current.statusLabel}</span>
        ...
        <div className="col-lg-8">
          <div className="card h-100">
            <div className="card-header">
              ...
              <span style={{ fontSize: '12px', color: '#9ca3af' }}>/topic/tickets/{ticketId}</span>
            </div>
        ...
        <div className="col-lg-4">
          ...
          {isAdmin || isMaintenance ? (
            <div className="card mb-3">
              ...
              <div className="card-body d-grid gap-3">
                {canAcceptWork ? (
                  <button className="btn btn-primary" onClick={() => acceptWork.mutate()} disabled={acceptWork.isPending}>
                    Nhận việc
                  </button>
                ) : null}
                {canResolve ? (
                ...
                {!canAcceptWork && !canResolve && !canCancel ? (
                  <div className="text-muted" style={{ fontSize: '13px' }}>
                    {current.status === 'RESOLVED'
                      ? 'Ticket đã hoàn tất, không còn thao tác trạng thái.'
                      : current.status === 'CANCELLED'
                        ? 'Ticket đã bị hủy.'
                        : 'Chỉ kỹ thuật viên được giao mới có thể nhận việc và xử lý ticket này.'}
                  </div>
                ) : null}
              </div>
            </div>
